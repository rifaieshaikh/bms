...
def _po_card(
    ...
    *,
    ...
    with st.container(border=True):
        ...
        if show_expected_date:
            st.caption(f"Expected: {_fmt_date(row.get('expected_date'))}")
...
def purchase_order_cards(
    ...
    *,
    ...
    render_card_grid(
        rows,
        lambda row, _idx: _po_card(
            row,
            suffix,
            view_label=view_label,
            view_full_width=view_full_width,
            show_expected_date=show_expected_date,
        ),
        suffix=suffix,
    )
