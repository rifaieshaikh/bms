...
def _computed_cols_changed(edited: dict, display: dict, *, show_gst: bool) -> bool:
    """True when disabled/computed columns differ and need a data_editor remount."""
    keys = ["Name", "HSN/SAC", "Rate", "Taxable", "Line Total"]
    if show_gst:
        keys.extend(["GST %", "CGST", "SGST", "UTGST", "IGST"])
    for key in keys:
        if not _values_equal(edited.get(key), display.get(key)):
            return True
    return False
...
def render_purchase_lines_editor(
    *,
    ...
    result_key = f"{key_prefix}_lines_result"
    ...
    @st.fragment
    def _purchase_lines_fragment() -> None:
        fragment_started = time.perf_counter()
        # Remounts stay inside this fragment so Vendor/dates outside do not flicker.
        if st.session_state.get(vendor_snap_key) != vendor_id:
            st.session_state[vendor_snap_key] = vendor_id
            force_vendor_rate = True
            # Vendor-specific rates must be re-resolved.
            rate_cache.clear()
        else:
        ...
        if use_item_select:
            ...
            if not products:
                st.warning(
                    "No active products in inventory. Add a product before creating lines."
                )
        ...
        if catalog_return_to:
            ...
            if add_cols[0].button("+ Add catalog item", key=f"{key_prefix}_add_catalog"):
                ...
                st.rerun()  # app scope — open catalog dialog on parent
        ...
        # Remount data_editor only when catalog/rate fill changes computed columns.
        needs_remount = False
        ...
        for index, raw in enumerate(edited_rows):
            ...
            if item_changed or is_new_item_row or force_vendor_rate:
                if _computed_cols_changed(raw, display_row, show_gst=show_gst):
                    needs_remount = True
        ...
        st.session_state[result_key] = (lines, gst_errors)

        if needs_remount:
            st.session_state.pop(editor_key, None)
            logger.debug(
                "po_lines.fragment remount duration_ms=%.1f",
                (time.perf_counter() - fragment_started) * 1000,
            )
            st.rerun(scope="fragment")

        logger.debug(
            "po_lines.fragment duration_ms=%.1f lines=%s remount=%s",
            (time.perf_counter() - fragment_started) * 1000,
            len(lines),
            needs_remount,
        )
        ...
        if unknown_items:
            ...
            for unknown in unknown_items:
                ...
                if st.button(
                    ...
                    st.rerun()  # app scope — open catalog dialog
