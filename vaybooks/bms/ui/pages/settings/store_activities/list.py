...
from vaybooks.bms.ui.list_schemas import STORE_ACTIVITIES

PENDING_EDIT_STORE_ACTIVITY = "pending_edit_store_activity"
...
@st.dialog("Add Store Activity")
def _add_activity_dialog(activity_service):
    name = st.text_input("Activity Name", key="add_store_act_name")
    category_value = st.selectbox(
        "Activity Type",
        _category_options(),
        key="add_store_act_category",
    )
    ...
    if meta["requires_pricing"]:
        ...
        hourly_expense = st.number_input(
            "Default Hourly Expense", min_value=0.0, key="add_store_act_expense"
        )
    else:
...
def _edit_activity_dialog(activity_service, activity_id: str):
    ...
    name = st.text_input(
        "Activity Name", value=activity.activity_name, key="edit_store_act_name"
    )
    ...
    custom_statuses_raw = st.text_area(
        ...
        placeholder="e.g.\nBilling\nPacking\nQuality Check",
        key="edit_store_act_statuses",
    )
    st.caption("Flow: " + " → ".join(activity.statuses))

    is_active = st.checkbox(
        "Active", value=activity.is_active, key="edit_store_act_active"
    )
...
def _activity_card(activity, index: int):
    with st.container(border=True):
        ...
        if st.button(
            "Edit",
            key=f"edit_store_act_btn_{index}_{activity.id}",
            width="stretch",
        ):
            st.session_state[PENDING_EDIT_STORE_ACTIVITY] = activity.id


def _load_activities(services, filters, sort):
    try:
        return services["store_activities"].list_activities(active_only=False)
    except Exception:
        return []
...
def _render_cards(page_activities, services):
    render_card_grid(
        ...
        suffix="store_activities",
    )


def render(services: dict):
    activity_service = services["store_activities"]
    bar = render_list(
        STORE_ACTIVITIES,
        services=services,
        ...
        primary_key="store_activities_add_btn",
        count_label="activities",
        empty_text="No store activities configured yet.",
        page_key_nav="store_activities_list",
    )
    if bar["primary_clicked"]:
        _add_activity_dialog(activity_service)

    pending = st.session_state.pop(PENDING_EDIT_STORE_ACTIVITY, None)
    if pending:
