"""Purchases Reports — Filters dialog (incl. date presets) and CSV download."""
...
_DATE_FIELD_BY_REPORT = {
    "Purchase Orders Pipeline": "order_date",
    "GRN Pending": "order_date",
    "Purchase Returns Summary": "return_date",
}

_DATE_CAPTION = {
    "Purchase Orders Pipeline": "Filtered by purchase order date.",
...
def _load_all(reports_svc, report_type: str) -> list:
    if report_type == "Purchase Orders Pipeline":
        return reports_svc.purchase_orders_pipeline()
    if report_type == "GRN Pending":
        return reports_svc.grn_pending()
    if report_type == "Purchases by Vendor":
...
def _render_table(
    ...
    page_rows, page, total_pages = paginate_list(
        data,
        page_key=f"purchases_report_page_{entity_key}",
    ...
    render_page_controls(
        ...
        len(data),
        page_key=f"purchases_report_page_{entity_key}",
        prev_key=f"purchases_report_prev_{entity_key}",
        next_key=f"purchases_report_next_{entity_key}",
        label="rows",
    )


def render(services: dict) -> None:
    st.header("Purchases Reports")

    reports_svc = services.get("reports_purchases")
    ...
    report_type = st.selectbox(
        "Report",
        options=PURCHASE_REPORT_TYPES,
        key="purchases_reports_type",
    )
